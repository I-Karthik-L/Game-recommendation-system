...
# ================================
# 0. Hugging Face Dataset Config
# ================================
HF_REPO_ID = "karthikkkkl/game_recommendation"
HF_REPO_TYPE = "model"  # this repo was created as a Model repo on Hugging Face, not a Dataset repo


def get_data_file(filename: str) -> str:
    """
    Downloads (and caches) a file from the Hugging Face dataset repo.
...
def load_data_strict():
    ...
    # --- Load Video Games ---
    try:
        vg_path = get_data_file("merged_data.csv")
        vg_df = pd.read_csv(vg_path, on_bad_lines='warn')
        ...
        status_log.append(f"❌ Video Games (merged_data.csv) Failed: {e}")

    # --- Load Board Games ---
    try:
        bgg_path = get_data_file("BGG_Data_Set.csv")
        bgg_df = pd.read_csv(bgg_path, encoding="ISO-8859-1", on_bad_lines='skip')
        ...
        status_log.append(f"❌ Board Games (BGG_Data_Set.csv) Failed: {e}")

    if not master_list:
        return None, None, None, status_log, None
    ...
    # Drop duplicate titles and reset the index BEFORE vectorizing, so df's
    # row positions stay perfectly aligned with tfidf_matrix's row positions.
    ...
    # Index Map (df is already deduped and index-reset above, so this aligns
    # directly with tfidf_matrix's row positions)
    indices = pd.Series(df.index, index=df['Title']).drop_duplicates()

    all_titles = sorted({str(t) for t in df['Title'].dropna().tolist()})

    return df, tfidf_matrix, indices, status_log, all_titles

# Load Data
df, tfidf_matrix, indices, status_log, all_titles = load_data_strict()
...
if df is not None:
    with st.sidebar:
        ...
        # Search
        search_term = st.text_input("🔍 Search for a game:", placeholder="Type to search...")

        if search_term:
            matches = [t for t in all_titles if search_term.lower() in t.lower()][:50]
        else:
            matches = all_titles[:50]

        if matches:
            selected_game = st.selectbox("🎯 Select a Game:", matches)
        else:
            selected_game = None
        ...
        if st.button("🚀 Launch Analysis", type="primary", use_container_width=True, disabled=selected_game is None):
    ...
    st.error("CRITICAL ERROR: No data found. Check the System Status panel in the sidebar for the exact reason.")